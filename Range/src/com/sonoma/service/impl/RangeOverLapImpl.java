 ...
 *This class implements RangeOverLap
 */
public class RangeOverLapImpl implements RangeOverLap{
	
	/* (non-Javadoc)
	 * @see com.sonoma.service.RangeOverLap#getRange(java.util.List)
	 * this method accepts range list in any order and converts it in one list that
	 * does not overlap
	 */
	public List<Range> getRange(List<Range> rangeList){
		...
		for(int i=0;i<rangeList.size()-1;i++) {
			...
			if((max >=rangeList.get(i+1).getStart()))
			{
